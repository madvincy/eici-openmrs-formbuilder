...
export class SaveFormService {
 ...
 constructor(private http: HttpClient, private sessionStorageService: SessionStorageService) {
  ...
  deleteClobData(valueReference: string) {
    return this.http.delete(`${this.baseUrl}/ws/rest/v1/clobdata/${valueReference}`, {headers: this.headers})
    .toPromise();
  }

  deleteResource(formUUID, resourceUUID) {
    return this.http.delete(`${this.baseUrl}/ws/rest/v1/form/${formUUID}/resource/${resourceUUID}`, {headers: this.headers})
    .toPromise();
  }
  ...
  uploadSchema(schema) {
    ...
    const schemaBlob = new Blob([JSON.stringify(schema)], {type: 'application/json'});
  ...
  saveNewForm(name, version, published?, description?, encounterType?) {
      ...
      return this.http.post(`${this.baseUrl}/ws/rest/v1/form`, body, {headers: this.headers})
      .pipe(map(res => JSON.parse(JSON.stringify(res))));
    }
    ...
    publish(uuid) {
        ...
        return this.http.post(`${this.baseUrl}/ws/rest/v1/form/${uuid}`, body, {headers: this.headers});
    }
    ...
    updateEncounterType(encounterType: any, formUuid: string)  {
        ...
        return this.http.post(`${this.baseUrl}/ws/rest/v1/form/${formUuid}`, body, {headers: this.headers});
    }
